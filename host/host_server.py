...
class HostHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            if path == "/api/status":
                status = api.status()
                self.send_json({"status": "online", "computer": status["host"]["name"], "authorization_required": True})
        ...
        try:
            ...
            if path == "/api/games/launch":
                ...
                if game is None:
                    ...
                    api.start_stream()
                    self.send_json({"ok": True, "pc": api.status()["host"]["name"], "state": "CONNECTING", "game": game_name})
                    return
